...
class Event(DoubanApiBase):
    ...
    def join(self, id, participate_date=''):
        return self._post('/v2/event/%s/participants'%id, participate_date=participate_date)

    def quit(self, id, participate_date=''):
        return self._delete('/v2/event/%s/participants'%id, participate_date=participate_date)
